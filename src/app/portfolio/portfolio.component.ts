...
interface Project {
  ...
  gradient: string;
  emoji: string;
  image?: string; // Optional project thumbnail image
}
...
export class PortfolioComponent {
  ...
  projects: Project[] = [
    {
      ...
      emoji: '🎨',
      image: 'assets/portfolio-screenshot.png' // Screenshot of the portfolio website
    }
  ];
  ...
  // type: 'live' for live site, 'github' for GitHub URL
  navigateWithAuth(project: Project, type: 'live' | 'github', event: Event) {
    event.preventDefault();
    
    const targetUrl = type === 'live' ? project.liveUrl : project.githubUrl;
    const label = type === 'live' ? 'Live Site' : 'GitHub Repository';
    
    if (targetUrl) {
      // Navigate to OTP auth page with redirect URL, project name, and type as query params
      this.router.navigate(['/auth'], {
        queryParams: {
          redirect: targetUrl,
          project: `${project.title} - ${label}`,
          type: type
        }
      });
    }
